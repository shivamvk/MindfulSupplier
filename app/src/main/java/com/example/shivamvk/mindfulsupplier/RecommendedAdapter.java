...
public class RecommendedAdapter extends RecyclerView.Adapter<RecommendedAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, int i) {
        ...
        viewHolder.tvLoadingTime.setText(order.getLoadingDate());
        ...
        if (APPLIEDORDERS.contains(orderid) && order.getCompleted().equals("No")){
        ...
        } else if (APPLIEDORDERS.contains(orderid) && order.getCompleted().equals("Yes") && supplier.equals(SharedPrefManager.getInstance(context).getEmail())){
            ...
            viewHolder.btApply.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    if(SharedPrefManager.getInstance(context).isEmailVerified().equals("No")){
                        AlertDialog.Builder builder =new AlertDialog.Builder(context);
                        builder.setMessage("Please verify your email before applying for any load");
                        builder.setPositiveButton("Okay", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialogInterface, int i) {
                                return;
                                            ...
                                            public void onComplete(@NonNull Task<Void> task) {
                                                if (task.isSuccessful()) {
                                                    DatabaseReference reference1 = FirebaseDatabase.getInstance()
                                                            .getReference("users")
                                                            .child(generateHash(order.getOrderby()))
                                                            .child("orders")
                                                            .child(generateHash(order.getLoadingPoint(),
                                                                    order.getTripDestination(),
                                                                    order.getTruckType(),
                                                            ...
                                                            .child("appliedby")
                                                            .child(generateHash(SharedPrefManager.getInstance(context).getEmail()));
                                                    reference1.setValue(SharedPrefManager.getInstance(context).getEmail());
                                                    dialog.dismiss();
                                                } else {
                                                    dialog.dismiss();
    ...
    public class ViewHolder extends RecyclerView.ViewHolder{
        ...
        public ViewHolder(@NonNull View itemView) {
            ...
            tvTripDestination = itemView.findViewById(R.id.tv_order_item_trip_destination);
            ...
            btApply = itemView.findViewById(R.id.bt_order_item_apply);
            tvOrderId = itemView.findViewById(R.id.tv_order_item_order_id);
            cardView = itemView.findViewById(R.id.cv_recommended);
